	...
	function renderxmlAction() {
		...
		if (count($potions) > 0) {
			foreach($potions as $p) {
				...
				$tab[] = "<div class='braltip'>".Bral_Helper_DetailPotion::afficherTooltip($potion)."<img src='".Zend_Registry::get('config')->static->url."/styles/braldahim_defaut/images/type_potion/type_potion_".$potion["id_type_potion"].".png' alt=\"".htmlspecialchars($potion["nom"]) ."\"/></div>";
