...
class Bral_Box_Tour extends Bral_Box_Box {
	...
	public function modificationTour() {
		 ...
		 * directement en position d'activité
		 */
		...
		if ($this->hobbit->est_ko_hobbit == "oui") { // KO
			Bral_Util_Log::tour()->debug(get_class($this)." KO du hobbit");
			$mdate = date("Y-m-d H:i:s");
			$this->hobbit->date_fin_tour_hobbit = Bral_Util_ConvertDate::get_date_add_time_to_date($mdate, $this->view->config->game->tour->inscription->duree_base_activite);
			$this->hobbit->date_debut_tour_hobbit = Bral_Util_ConvertDate::get_date_remove_time_to_date($mdate, $this->view->config->game->tour->inscription->duree_base_activite);
			$this->hobbit->date_fin_latence_hobbit = $mdate;
			$this->hobbit->tour_position_hobbit = $this->view->config->game->tour->position_activite;
			$this->is_update_tour = true;
			...
			$this->is_update_tour = true;
		} elseif(($date_courante < $this->hobbit->date_fin_latence_hobbit) // Sommeil
		&& $this->is_nouveau_tour) {
			Bral_Util_Log::tour()->debug(get_class($this)." Position Sommeil Tour");
			$this->hobbit->tour_position_hobbit = $this->view->config->game->tour->position_latence;
			$this->hobbit->pa_hobbit = 0;
			$this->is_update_tour = true;
		} elseif($date_courante >= $this->hobbit->date_fin_latence_hobbit // Activité
		&& ( (!$this->is_nouveau_tour && ($this->hobbit->tour_position_hobbit != $this->view->config->game->tour->position_activite))
		|| ($this->is_nouveau_tour))) {
			Bral_Util_Log::tour()->debug(get_class($this)." Position Activite Tour");
			$this->hobbit->tour_position_hobbit = $this->view->config->game->tour->position_activite;
			$this->hobbit->pa_hobbit = $this->view->config->game->pa_max;
			$this->is_update_tour = true;
		}
	...
	private function calculInfoTour() {
		...
		if ($this->view->user->tour_position_hobbit == $this->view->config->game->tour->position_latence) {
			$info = "Fin Sommeil &agrave; ".$this->hobbit->date_fin_latence_hobbit;
		} else {
			$info = "Fin Activité &agrave; ".$this->hobbit->date_fin_tour_hobbit;
		}
