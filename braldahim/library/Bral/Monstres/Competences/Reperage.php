...
abstract class Bral_Monstres_Competences_Reperage extends Bral_Monstres_Competences_Competence {
	...
	public static function peutAttaquer($cible, $monstre) {
		Bral_Util_Log::viemonstres()->trace("Bral_Monstres_Competences_Reperage - peutAttaquer - enter - (idm:".$monstre["id_monstre"].") cible:".$cible["id_hobbit"]);
		...
		if ($nbAttaqueEsquivee >= 3 && $cible["niveau_hobbit"] > $monstre["niveau_monstre"] + 5) {
			...
			if ($de < $diffNiv) {
				$retour = false;
				Bral_Util_Log::viemonstres()->trace("Bral_Monstres_Competences_Reperage - peutAttaquer - (idm:".$monstre["id_monstre"].") ne peut pas attaquer ".$cible["id_hobbit"]);
			} else {
				$retour = true;
			}
		}
		...
		if ($retour) {
			Bral_Util_Log::viemonstres()->trace("Bral_Monstres_Competences_Reperage - peutAttaquer - (idm:".$monstre["id_monstre"].") peut attaquer ".$cible["id_hobbit"]);
		}
