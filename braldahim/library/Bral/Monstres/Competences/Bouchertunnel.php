...
class Bral_Monstres_Competences_Bouchertunnel extends Bral_Monstres_Competences_Prereperage {
	...
	public function actionSpecifique() {
		...
		$cible = $braldunTable->findBraldunAvecRayon($this->monstre["x_monstre"], $this->monstre["y_monstre"], $this->monstre["z_monstre"], $vue, null, true);
		...
		Bral_Util_Log::viemonstres()->trace(get_class($this)." - Bouchertunnel - (idm:".$this->monstre["id_monstre"].") - nbMonstre en vue:".count($monstres));
		...
		if ($cible == null || count($cible) < 1) {
			...
			if ($nids != null && count($nids) > 0) { // S'il y a des nids sur la case
				...
				Bral_Util_Log::viemonstres()->trace(get_class($this)." - Bouchertunnel - (idm:".$this->monstre["id_monstre"].") - braldun non vu, suppression de tunnel x:".$this->monstre["x_monstre"]. " y:".$this->monstre["y_monstre"]." z:".$this->monstre["z_monstre"]);
				...
				Bral_Util_Log::viemonstres()->trace(get_class($this)." - Bouchertunnel - (idm:".$this->monstre["id_monstre"].") - braldun non vu, pas suppression de tunnel. Nb monstre sur case hors groupe :".count($monstres). " x:".$this->monstre["x_monstre"]. " y:".$this->monstre["y_monstre"]." z:".$this->monstre["z_monstre"]);
				$retour = Bral_Monstres_Competences_Prereperage::SUITE_DISPARITION;
			}
