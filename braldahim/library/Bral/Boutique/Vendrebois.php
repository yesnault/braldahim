...
class Bral_Boutique_Vendrebois extends Bral_Boutique_Boutique {
	...
	function prepareCommun() {
		...
		if ($charretteRowset != null && count($charretteRowset) == 1) {
			$this->view->charretteOk = true;
			$charrette = $charretteRowset[0];
			$tabCharrette = array(
				"nb_rondin" => $charrette["quantite_rondin_charrette"],
			);
		} else if (count($charretteRowset) > 1) {
			throw new Zend_Exception(get_class($this)."::count(charretteRowset) > 1 :".count($charretteRowset). " idh=".$this->view->user->id_hobbit);
		}
