...
class Bral_Util_Vue {
	...
	private static function data(&$view) {
		...
		$regions = $regionTable->selectVue($view->x_min, $view->y_min, $view->x_max, $view->y_max);
		...
		for ($j = $centre_y_max; $j >= $centre_y_min; $j--) {
			for ($i = $centre_x_min; $i <= $centre_x_max; $i++) {
				...
				$tableau["Cases"][] = array("X" => $display_x, "Y" => $display_y, "Fond" => "plaine");
			}
		}
