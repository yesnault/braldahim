...
class Bral_Competences_Ramasser extends Bral_Competences_Competence {
	...
	function prepareCommun() {
		...
		$typesElements[1] = array("id_type_element" => 1, "selected" => $id_type_courant, "nom_systeme" => "equipements", "nom_element" => "Equipements");
		$typesElements[2] = array("id_type_element" => 2, "selected" => $id_type_courant, "nom_systeme" => "munitions", "nom_element" => "Munitions");
		...
		if (count($runes) > 0) {
			...
			foreach ($runes as $r) {
				$tabRunes[$r["id_rune_element_rune"]] = array(
					...
					"est_identifiee" => $r["est_identifiee_element_rune"],
				);
			}
		} else {
			$this->view->ramasserOk = false;
	...
	private function prepareTypeAutres() {
		...
		$nbCastarPossible = floor($poidsRestant / Bral_Util_Poids::POIDS_CASTARS);

		if ($nbPeauPossible > 0 || $nbViandePossible > 0 || $nbViandePrepareePossible > 0 ||
		$nbRationPossible > 0 || $nbCuirPossible > 0 || $nbFourrurePossible > 0 || $nbCastarPossible > 0 || $nbPlanchePossible > 0) {
			$this->view->poidsPlaceDisponible = true;
		} else {
			$this->view->poidsPlaceDisponible = false;
		}
		...
		if (count($elements) == 1) {
			foreach ($elements as $e) {
				...
				if ($e["quantite_castar_element"] > 0) $tabAutres[8] = array("prefix" => "un", "nom" => "Castar", "nom_systeme" => "quantite_castar" , "pluriel" => "Castars", "nb" => $e["quantite_castar_element"], "nbPossible" => $nbCastarPossible);
	...
	private function ramasseTypeAutres() {
		...
		if ($autre["nom_systeme"] == "quantite_castar") {
