...
class Bral_Competences_Forger extends Bral_Competences_Competence {
	...
	private function calculForger($idTypeEquipement, $niveau, $nbRunes) {
		...
		if (Bral_Util_Commun::isRunePortee($this->view->user->id_braldun, "GE")) { // s'il possede une rune GE
		...
		if (count($recetteEquipement) > 0) {
			...
			$dataFabricants["id_fk_metier_stats_fabricants"] = Bral_Util_Metier::METIER_FORGERON_ID;
