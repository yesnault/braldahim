 ...
 */
class AdministrationfilonController extends Zend_Controller_Action
{
	...
	private function filonsPrepare()
	{
		...
		foreach ($zonesRowset as $z) {
			$nombreFilons = $filonTable->countVue($z ["x_min_zone"], $z ["y_min_zone"], $z ["x_max_zone"], $z ["y_max_zone"], 0);
			$nombreCases = ($z ["x_max_zone"] - $z ["x_min_zone"]) * ($z ["y_max_zone"] - $z ["y_min_zone"]);
			$couverture = ($nombreFilons * 100) / $nombreCases;
			$zones [] = array("id_zone" => $z ["id_zone"], "x_min" => $z ["x_min_zone"], "x_max" => $z ["x_max_zone"], "y_min" => $z ["y_min_zone"], "y_max" => $z ["y_max_zone"], "environnement" => $z ["nom_environnement"], "nombre_filons" => $nombreFilons, "nombre_cases" => $nombreCases, "couverture" => round($couverture));
		}

		foreach ($typeMineraiRowset as $t) {
			$typeMinerais [] = array("id_type_minerai" => $t->id_type_minerai, "nom" => $t->nom_type_minerai, "nom_systeme" => $t->nom_systeme_type_minerai, "description" => $t->description_type_minerai);
		}
