...
class CharretteMinerai extends Zend_Db_Table {
	...
	function findByIdHobbit($id_hobbit) {
		...
		->from('type_minerai', '*')
		->where('id_fk_hobbit_charrette_minerai = '.intval($id_hobbit))
	...
	function insertOrUpdate($data) {
		...
		->where('id_fk_hobbit_charrette_minerai = ?',$data["id_fk_hobbit_charrette_minerai"])
		...
		if (count($resultat) == 0) { // insert
			...
			$where = ' id_fk_type_charrette_minerai = '.$data["id_fk_type_charrette_minerai"];
			$where .= ' AND id_fk_hobbit_charrette_minerai = '.$data["id_fk_hobbit_charrette_minerai"];
