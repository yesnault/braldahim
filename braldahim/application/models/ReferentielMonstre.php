...
class ReferentielMonstre extends Zend_Db_Table {
	...
	public function findAll() {
		...
		->order(array('id_type_monstre', 'id_taille_monstre'));
		$sql = $select->__toString();
		return $db->fetchAll($sql);
	}
	
	public function findByTailleAndType($idTaille, $idType) {
		$db = $this->getAdapter();
		$select = $db->select();
		$select->from('ref_monstre', '*')
		->from('type_monstre', '*')
		...
		->where('ref_monstre.id_fk_taille_ref_monstre = taille_monstre.id_taille_monstre')
		->where('type_monstre.id_fk_type_groupe_monstre = type_groupe_monstre.id_type_groupe_monstre')
		->where('ref_monstre.id_fk_taille_ref_monstre = ?', $idTaille)
		->where('ref_monstre.id_fk_type_ref_monstre = ?', $idType);
