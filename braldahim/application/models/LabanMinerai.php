...
class LabanMinerai extends Zend_Db_Table {
	...
	protected $_primary = array('id_hobbit_laban_minerai', 'id_fk_type_laban_minerai');
	...
	function findByIdHobbit($id_hobbit) {
		...
		->from('type_minerai', '*')
		->where('id_hobbit_laban_minerai = '.intval($id_hobbit))
	...
	function insertOrUpdate($data) {
		...
		->where('id_hobbit_laban_minerai = ?',$data["id_hobbit_laban_minerai"])
		->group('quantite');
		$sql = $select->__toString();
