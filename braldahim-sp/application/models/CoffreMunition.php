 ...
 *
 * $Id: CoffreMunition.php 2019 2009-09-19 10:32:13Z yvonnickesnault $
...
class CoffreMunition extends Zend_Db_Table {
	...
	protected $_primary = array('id_fk_hobbit_coffre_munition', 'id_fk_type_coffre_munition');
	...
	function findByIdHobbit($id_hobbit) {
		...
		->from('type_munition', '*')
		->where('id_fk_hobbit_coffre_munition = '.intval($id_hobbit))
	...
	function insertOrUpdate($data) {
		...
		->where('id_fk_hobbit_coffre_munition = ?',$data["id_fk_hobbit_coffre_munition"])
		...
		if (count($resultat) == 0) { // insert
			...
			$where = ' id_fk_type_coffre_munition = '.$data["id_fk_type_coffre_munition"];
			$where .= ' AND id_fk_hobbit_coffre_munition = '.$data["id_fk_hobbit_coffre_munition"];
