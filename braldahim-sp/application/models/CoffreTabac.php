 ...
 *
 * $Id: CoffreTabac.php 2019 2009-09-19 10:32:13Z yvonnickesnault $
...
class CoffreTabac extends Zend_Db_Table {
	...
	protected $_primary = array('id_fk_hobbit_coffre_tabac', 'id_fk_type_coffre_tabac');
	...
	function findByIdHobbit($id_hobbit) {
		...
		->from('type_tabac', '*')
		->where('id_fk_hobbit_coffre_tabac = '.intval($id_hobbit))
	...
	function insertOrUpdate($data) {
		...
		->where('id_fk_hobbit_coffre_tabac = ?',$data["id_fk_hobbit_coffre_tabac"])
		...
		if (count($resultat) == 0) { // insert
			...
			$where = ' id_fk_type_coffre_tabac = '.$data["id_fk_type_coffre_tabac"];
			$where .= ' AND id_fk_hobbit_coffre_tabac = '.$data["id_fk_hobbit_coffre_tabac"];
