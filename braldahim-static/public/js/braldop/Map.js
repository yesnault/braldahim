function Map(canvasId, posmarkid, dialogId) {
    ...
    if (this.canvas.addEventListener) this.canvas.addEventListener("DOMMouseScroll", function(e) {
        e.preventDefault(), _this.mouseWheel(e)
    }, false); // firefox
    ...
    for (var ic = 0; ic < this.mapData.Couches.length; ic++) {
        ...
        if (couche.Palissades) {
            ...
            for (var i = couche.Palissades.length; i-- > 0;) {
                ...
                var c;
                var nb = 0;
                if ((c = this.getCell(couche, p.X + 1, p.Y)) && (c.palissade)) {
                    p.sides |= B_RIGHT;
                    nb++;
                }
                if ((c = this.getCell(couche, p.X - 1, p.Y)) && (c.palissade)) {
                ...
                if ((c = this.getCell(couche, p.X, p.Y + 1)) && (c.palissade)) {
                    p.sides |= B_TOP;
                    nb++;
                }
                if ((c = this.getCell(couche, p.X, p.Y - 1)) && (c.palissade)) {
